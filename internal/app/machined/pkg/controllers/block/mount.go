	...
	for {
		...
		for mountRequest := range mountRequests.All() {
			...
			if !(volumeNotReady || mountRequestTearingDown) && mountParentReady {
				...
				if err = ctrl.handleMountOperation(ctx, logger, childTargets, rootPath, mountSource, mountTarget, mountFilesystem, mountRequest, volumeStatus); err != nil {
					return err
				}
...
func (ctrl *MountController) handleMountOperation(
	...
	case block.VolumeTypeExternal:
		return ctrl.handleDiskMountOperation(ctx, logger, childTargets, mountSource, filepath.Join(rootPath, mountTarget), mountFilesystem, mountRequest, volumeStatus)
