...
func (suite *FSScrubScheduleSuite) createVolumeStatus(id string, mutate func(*block.VolumeStatusSpec)) {
	...
	volumeStatus.TypedSpec().ScrubPeriod = testScrubPeriod
...
func (suite *FSScrubScheduleSuite) TestEligibility() {
	...
	ctest.AssertResource(suite, constants.EphemeralPartitionLabel, func(schedule *block.FSScrubSchedule, asrt *assert.Assertions) {
		...
		asrt.Equal(testScrubPeriod, schedule.TypedSpec().Period)
		asrt.True(schedule.TypedSpec().NextScrub.After(now), "next scrub should be in the future")
		asrt.True(schedule.TypedSpec().NextScrub.Before(now.Add(testScrubPeriod).Add(time.Second)), "next scrub should be within one period")
	})
...
func (suite *FSScrubScheduleSuite) TestScheduleIsSaltedWithNodeID() {
	...
	ctest.AssertResource(suite, constants.EphemeralPartitionLabel, func(schedule *block.FSScrubSchedule, asrt *assert.Assertions) {
		saltedOffset := block.ScheduleOffset(testScrubNodeID+"/"+constants.EphemeralPartitionLabel, testScrubPeriod)

		asrt.Zero(schedule.TypedSpec().NextScrub.Sub(time.Unix(0, int64(saltedOffset))) % testScrubPeriod)
	})
}
