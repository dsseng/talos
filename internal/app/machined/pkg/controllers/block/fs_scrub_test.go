...
// runnerScrubPeriod is kept short so the next scrub slot is reached quickly in tests.
const runnerScrubPeriod = 2 * time.Second
...
func (suite *FSScrubSuite) createSchedule(id string) {
	...
	schedule.TypedSpec().Period = runnerScrubPeriod
	schedule.TypedSpec().NextScrub = block.NextScheduledTime(id, runnerScrubPeriod, time.Now())
...
func (suite *FSScrubSuite) TestScrubMounted() {
	...
	ctest.AssertResource(suite, "volume", func(status *block.FSScrubStatus, asrt *assert.Assertions) {
		...
		asrt.Equal(runnerScrubPeriod, status.TypedSpec().Period)
...
func (suite *FSScrubSuite) TestSkipNotMounted() {
	...
	suite.Assert().Never(func() bool {
		return suite.tracker.count() > 0
	}, 2*runnerScrubPeriod, 100*time.Millisecond)
