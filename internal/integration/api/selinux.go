			...
			for _, p := range procs {
				...
				case "systemd-udevd":
					suite.Require().Contains(
						...
						"system_u:system_r:udev_t:s0",
					)
				case "dashboard":
					suite.Require().Contains(
						suite.getLabel(nodeCtx, p.Pid),
						"system_u:system_r:dashboard_t:s0",
					)
				case "containerd":
					if strings.Contains(p.Args, "/system/run/containerd") {
						suite.Require().Contains(
							suite.getLabel(nodeCtx, p.Pid),
							"system_u:system_r:sys_containerd_t:s0",
						)
					} else {
						suite.Require().Contains(
							suite.getLabel(nodeCtx, p.Pid),
							"client_u:client_r:pod_containerd_t:s0",
						)
					}
				case "init":
					suite.Require().Contains(
				...
				case "kubelet":
					suite.Require().Contains(
						suite.getLabel(nodeCtx, p.Pid),
						"client_u:client_r:kubelet_t:s0",
					)
				case "apid":
					suite.Require().Contains(
						suite.getLabel(nodeCtx, p.Pid),
						"system_u:system_r:apid_t:s0",
					)
				case "trustd":
					suite.Require().Contains(
						suite.getLabel(nodeCtx, p.Pid),
						"system_u:system_r:trustd_t:s0",
					)
				}
			}
		}
